...
function CreateBlog() {
  ...
  useEffect(() => {
    if (isHighlight) {
      ...
      document.getElementById("projects-checkbox-container").style.display =
        "none";
      document.getElementById("featured-checkbox-container").style.display =
        "none";
      document.getElementById("highlights-checkbox-container").style.display =
        "none";
    }
  }, [isHighlight]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!isProject) {
        if (update && !isDelete) {
          endpoint = ApiBaseUrl + "update-blog";
        } else if (isDelete) {
          endpoint = ApiBaseUrl + "delete-blog";
        } else {
          endpoint = ApiBaseUrl + "create-blog";
        }
      }
      if (isProject) {
        if (update && !isDelete) {
          endpoint = ApiBaseUrl + "update-project";
        } else if (isDelete) {
          endpoint = ApiBaseUrl + "delete-project";
        } else {
          endpoint = ApiBaseUrl + "create-project";
        }
      }
      if (isHighlight) {
        if (update && !isDelete) {
          endpoint = ApiBaseUrl + "update-highlight";
        } else if (isDelete) {
          endpoint = ApiBaseUrl + "delete-highlight";
        } else {
          endpoint = ApiBaseUrl + "create-highlight";
        }
      }
  ...
  return (
    <div id="creator-page">
            ...
            {Object.keys(blogs).map((key) => {
              return (
                <p
                  className="data-row"
                  onClick={() => {
                    fillBlogData(blogs[key]);
                    setTimeStamp(blogs[key].time_stamp);
                  }}
            ...
            {Object.keys(projects).map((key) => {
              return (
                <p
                  className="data-row"
                  onClick={() => {
                    fillProjectData(projects[key]);
                    setTimeStamp(projects[key].time_stamp);
                  }}
                  key={projects[key].time_stamp}
                >
            ...
            {Object.keys(highlights).map((key) => {
              return (
                <p
                  className="data-row"
                  onClick={() => {
                    fillHighlightsData(highlights[key]);
                    setTimeStamp(highlights[key].time_stamp);
                  }}
      ...
      <form id="form" onSubmit={handleSubmit}>
        <h1 style={{ margin: "0px" }} id="form-title">
          {formMode}
        </h1>
        ...
        <div id="submit-btn-container">
          ...
          <div id="buttons-container">
            <button id="clear-button" onClick={() => clearInputFields()}>
              Clear
            </button>
